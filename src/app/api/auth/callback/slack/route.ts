...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const response = await fetch('https://slack.com/api/oauth.v2.access', {
      ...
      //   Send the code, client ID, client secret, and redirect URI as the body of the request
    ...
    if (!!data?.ok) {
      ...
      return NextResponse.redirect(
        `https://autofloww.vercel.app/connections?app_id=${appId}&authed_user_id=${userId}&authed_user_token=${userToken}&slack_access_token=${accessToken}&bot_user_id=${botUserId}&team_id=${teamId}&team_name=${teamName}`,
      );
    }
  } catch (error) {
    console.error(error);
